...
| `data/fragmentation_propensity_glm.rds` | **Exception — ships committed in git, not gitignored.** The Calibrated scoring mode's model: a single joint logistic regression (`scripts/build_propensity_glm_model.R`), replacing an earlier version hand-assembled from three separately-calibrated pieces multiplied together (never validated as a whole — cross-study holdout AUROC was ~0.64 for the assembly vs. ~0.78–0.82 for one joint fit on the same features). Its trainer reads from scratch feature caches that were never committed, so it isn't reproducible from a fresh clone; the trained model is checked in directly instead (see `.gitignore`'s explicit exception for it). Without this file, Calibrated mode fails with "Calibrated (GLM) scoring mode unavailable". |
| `data/fragmentation_propensity_rf.rds` | **Exception — this one ships committed in git, not gitignored.** Its trainer (`scripts/build_propensity_rf_model.R`) reads from scratch feature caches that were never committed, so it isn't reproducible from a fresh clone; the trained model is checked in directly instead (~20MB, see `.gitignore`'s explicit exception for it). Also requires the `ranger` package (in `renv.lock`). Without this file, RF mode — the default scoring mode — fails with "RF ranking mode unavailable"; the app falls back to working fine in Calibrated (GLM) mode only. |
